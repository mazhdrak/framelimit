...
- [x] Корекция на CPU, GPU, VRAM, TGP, RAM, display, weight и battery.
  ...
  - [x] Девети high-risk source-metadata batch: всичките 34 активни catalog записа вече изискват model code, HTTPS spec source и ISO дата на проверката; Titan 18, Raider 18, SCAR 18/16, Alienware 18 Area-51, Zephyrus G16 GU605CW и Alienware 16X Aurora са сверени с официалните MSI/ASUS/Dell спецификации, включително корекции на TGP, weight, battery, display и CPU полета.
  - [x] Десети spec-gap batch: всеки оставащ неизвестен TGP, weight или battery запис вече има конкретно обяснение в `specGaps`; audit-ът блокира както недокументирани `null` стойности, така и остарели gap бележки след попълване на поле, дублираната HP Omen дата е премахната, а TGP и display database summaries са синхронизирани с пълния sourced catalog.
...
- [x] Обновяване на стари цени и добавяне на дата на проверката.
  ...
  - [x] Dataset license batch: RTX 50 TGP Database JSON-LD вече декларира `FRAMELIMIT Data License 1.0` чрез публичен versioned URL и `isAccessibleForFree`; `audit-tgp-database.mjs` пази license metadata и видимата license секция в methodology.
  ...
  - [x] Тридесет и втори reference-price date batch: всичките 27 налични editorial reference цени вече имат собствена ISO дата `priceCheckedAt`; guide audit-ът блокира както недатирани цени, така и дата без цена, а finder и shared price UI показват конкретната дата отделно от датата на retail-link проверката.
...
- [x] Свързване на individual reviews с подходящия price guide.
  ...
  - [x] Двадесети RTX 5080 comparison cluster batch: Omen Max 16 B64BNUA#ABA срещу Vector 16 A2XWIG-058US е свързан двупосочно с двата canonical reviews, като регионалният Omen benchmark остава отделен от exact US retail SKU-а.
...
- [x] Свързване на reviews с GPU и technology guides.
  ...
  - [x] Седемнадесети RTX 5080 comparison technology-link batch: Omen Max 16 и Vector 16 reviews се валидират спрямо TGP и DLSS/Frame Generation базите заедно с exact-SKU comparison backlinks.
  ...
  - [x] Двадесет и втори Radeon-vs-RTX technology-link batch: Flow Z13 и G14 reviews се валидират спрямо RTX-vs-AMD explainer-а и DLSS/FSR/Frame Generation базата заедно с exact-SKU comparison backlinks.
  - [x] Двадесет и трети site-wide technology coverage batch: всичките 29 индексируеми product reviews се валидират автоматично за DLSS/FSR/Frame Generation backlink и за подходящ RTX TGP или RTX-vs-AMD GPU guide; `noindex` aliases и `review-notes` са изключени.
...
- [x] Избягване на ръчно публикувани цени без надежден процес за обновяване; reference стойностите са отделени от live API офертите, имат собствена дата и автоматично се скриват от price UI/finder след 30 дни чрез `scripts/audit-price-freshness.mjs` и shared freshness helper.
...
Следващ приоритет: завършване на distribution/backlink sprint-а чрез
moderator-first запитване към r/GamingLaptops, проследяване на editorial
отговорите и след това Search Console baseline при поне 28 дни данни; без
купени линкове и без generic guide expansion.

Distribution readiness update: добавени са generated `CITATION.cff`,
Frictionless `datapackage.json`, dataset README и персонализируем outreach
backlog. GitHub и Kaggle публикациите са live; editorial outreach-ът е изпратен.
Единствената community публикация остава pending до изрично moderator
разрешение.
