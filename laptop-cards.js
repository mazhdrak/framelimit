...
function flInjectStyles() {
  ...
  style.textContent = `
/* ── FL CARD IMAGE ── */
.fl-img-wrap{position:relative;height:220px;overflow:hidden;margin:-24px -24px 20px -24px;background:linear-gradient(135deg,#0d1520 0%,#060c12 100%)}
.fl-img-wrap img{width:100%;height:100%;object-fit:cover;object-position:center;transition:transform .5s ease;display:block}
...
/* ── HERO PICK IMAGE COL ── */
.hp-img-col{width:110px;min-width:110px;height:72px;background:linear-gradient(135deg,#0d1520,#060c12);display:flex;align-items:center;justify-content:center;overflow:hidden;border-right:1px solid var(--border,#1E2A36);flex-shrink:0}
.hp-img-col img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .3s}
.hp-card:hover .hp-img-col img{transform:scale(1.06)}

/* ── DEAL CARD IMAGE ── */
.deal-img{height:160px;overflow:hidden;margin:-20px -20px 16px -20px;border-bottom:1px solid #1E2A36;background:linear-gradient(135deg,#0d1520,#060c12)}
.deal-img img{width:100%;height:100%;object-fit:cover;object-position:center;display:block;transition:transform .4s}
