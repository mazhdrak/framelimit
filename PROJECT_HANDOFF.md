...
- Added a `20260911-1` cache-busting version to every page that loads `laptops.js`, and to the Reviews hub loader. Production initially served the old catalog name after deployment; the versioned URLs force the corrected normal product names and consolidated record to load.
...
## Reviews hub editorial and responsive-design cleanup — September 11, 2026

Applied the normal-name policy across the Reviews hub and the affected full reviews. Public titles, cards, verdicts, CTA labels and descriptive copy no longer expose Amazon ASINs or retailer/factory suffixes. Identifiers remain in Amazon hrefs, internal catalog records and technical datasets where they are required for offer integrity or configuration control.

- Replaced promotional or unsupported card labels and claims with factual configuration summaries. Removed unverified superlatives, exact price comparisons, absolute throttling statements, exact fan-noise figures, battery-runtime claims and percentage gaps from the hub. Reviews remain N/R where no matching controlled test supports a score.
- Cleaned the Lenovo Legion 5i Gen 10 and Lenovo LOQ 15 Gen 10 full pages so readers see ordinary product names and configuration language. The LOQ page no longer publishes its historical individual amount; the tagged Amazon link remains.
- Expanded `scripts/audit-public-retail-identifiers.mjs` to inspect rendered catalog fields and additional card/verdict/spec copy. It now catches ASINs and retail model identifiers that could otherwise reappear through `laptops.js` hydration.
- Improved review-card scanning without changing the site's visual system: larger spec chips, stronger emphasis on the first four characteristics, and a readable two-column mobile spec grid with full-width CTAs. Disabled the floating scroll controls on narrow screens because they covered the specification chips.
- Removed hard-coded review-count claims from the hub so the copy cannot drift from the actual catalog. Cache versions are `style.css?v=20260911-1` on the Reviews hub and `laptops.js?v=20260911-3` across pages that hydrate catalog content.
- Visual QA covered the Reviews hub on desktop and at a 390x844 mobile viewport. Product name, image, N/R state, Amazon CTA and the first four specifications remain readable and prominent. This is targeted visual QA, not a full WCAG certification.
- Validation: 25/26 audit scripts pass. `audit-price-freshness.mjs` correctly reports the same 24 reference observations as 58 days old; their dates were not changed. Retail-price and budget-offer regression tests pass. The Amazon audit reports 40 managed ASINs and 196 direct HTML Amazon links. Private design notes remain under `audit-site-2026-09-08/` and must not be staged.

## Budget-offer refresh and link-only price report — September 11, 2026

Rechecked the stale retail references in Amazon US and ASUS US with delivery location set to ZIP 10001. The private evidence file `audit-site-2026-09-08/stale-reference-price-check-2026-09-11.json` contains 31 observations, including condition, availability, seller and checkout amount where exposed. Keep this file untracked and do not copy its individual prices into public pages. The observed status mix was 5 in stock, 6 low stock, 1 available, 11 unavailable, 2 used-only, 2 with no new offer, 1 exact price unconfirmed and 3 blocked.

- Replaced all 24 stale July catalog observations: valid new offers now carry a September 11 internal reference date, while unavailable, used-only, blocked or unverifiable records have null price/date values. Added current internal reference records for the MSI Vector RTX 5080 and Acer Nitro V 16S where required by the budget guides. Price freshness now reports 9 dated observations and zero errors.
- Rebuilt the budget-guide eligibility from the checked offer totals while keeping individual amounts private. Current eligible order is HP Victus 15 under 1,000; Acer Nitro V 16S under 1,500; Acer Nitro V 16S, Gigabyte Gaming A16 RTX 5070 and MSI Vector 16 HX AI RTX 5070 Ti under 2,000; ASUS ROG Flow Z13, MSI Katana 15 HX and Alienware 16X Aurora under 2,500; MSI Vector RTX 5080 under 3,000. Other checked products remain in watchlists when unavailable, used-only, above the ceiling or lacking an exact confirmed offer.
- Preserved the public naming policy: product cards and quick picks use ordinary names such as `ASUS TUF Gaming A16`; ASINs and retailer/factory suffixes remain limited to internal link data, technical evidence and private verification files.
- Converted the July price-report page into a link-only retail coverage report. It renders catalog counts, GPU counts, check status, snapshot status and retailer links without showing individual or median product prices or public model codes. Public dollar figures on the page are limited to navigation labels for the budget-guide thresholds.
...
- Published in `9e859ce`, pushed to `origin/main`. Live HTTP checks confirmed all 26 changed review URLs return 200 with the new stylesheet reference and buying panel; all five synthetic-result blocks are present. The versioned stylesheet also returns 200 with the mobile table correction. Private verification: `review-layout-2026-09-15/production-verification.json` inside the existing audit folder.
